...
4. **Phase 3 — Schema rename + drops** — split into 3a (safe rename) and 3b (model drops, must follow Phase 4 jobcards rework):
   ...
   - **3b — drop cut models:** ⏸️ **deferred under the UI-only 4b decision.** Because we kept the working backend, several "cut" models are still actively written by it — `WastageRecord` (stage wastage), `DustLot` (dust lots), `StockLedgerEntry` (reconcile stock postings) — so dropping them would break the retained engine. Truly-orphaned models (`Customer`, `Vendor`, `Order`, `Estimate`/`EstimateLine`, `Assembly`, `QCInspection`, `CashBankLedgerEntry`, `CustomerLedgerEntry`, `ChargeType`, `CostingEditLog`) could be dropped in a later pass, but that also means removing the now-caller-less `estimates` module + its pdf/excel/service files. Left as optional cleanup; not required for the simplified app. The unused models are invisible to the client.
5. **Phase 4** — split into 4a (done) and 4b (large, pending a decision):
   ...
   - **4b — reconcile-model rewrite (NOT a field-strip):** the build implements a *different costing paradigm* than the spec — gold-loss accounting (chizzat, wastage-exceptions, dust-lots, gold-scrap, stock-ledger, over-reconciliation), ~180 refs across 13 files. Full swap to the spec's weight-accumulation model would be a ground-up engine re-implementation (highest risk, financial ledger). **Decision (signed off): UI-only simplification** — make the UX simple without touching the working backend.
     - **4b-UI:** ✅ **DONE** (commits `a2329dc`, `6435eb5`). Reconcile form → just Finished Weight + Dust (backend defaults the 9 removed fields to 0). Job-card page decluttered: removed the broken DispatchModal, the null "Linked Estimate" + "Chizzat" panels, gold Job-Details fields; MaterialAccountabilityPanel 7 gold metrics → 4 silver (Issued/In Piece/Dust/Wastage). Verified: API boots, create-job-card-from-Item-Master works end-to-end (JOB-2026-0001, 5 stages), tsc + lint clean, no dead links/endpoints.
     - **Deferred (not needed for UI-only):** full weight-accumulation math (spec §4 REPLACE/ADD), removing the backend chizzat/exception/dust-lot engine, Phase 5's 220g→206.780g verification. Revisit only if the client needs the exact spec costing math.
